...
class BestCourseDataSourceImpl : BestCourseDataSource {
    ...
    override fun findBestBuyCurrencies(courses: Set<Bank>): Map<CurrencyName, Bank> = currencyKeys
    ...
    override fun findBestSellCurrencies(courses: Set<Bank>): Map<CurrencyName, Bank> = currencyKeys
        .associateWith { currencyKey ->
            courses
                .asSequence()
    ...
    private fun isSellRateCorrect(currency: Bank, currencyKey: CurrencyName): Boolean {
        val rate = currency.resolveCurrencySellRate(currencyKey)
        return rate.isNotEmpty() && rate != UNKNOWN_COURSE
    }

    private fun isBuyRateCorrect(currency: Bank, currencyKey: CurrencyName): Boolean {
        val rate = currency.resolveCurrencyBuyRate(currencyKey)
        return rate.isNotEmpty() && rate != UNKNOWN_COURSE
    }

    private inline fun <K, V> ScatterSet<K>.associateWith(
        valueSelector: (K) -> V
    ): ScatterMap<K, V> {
